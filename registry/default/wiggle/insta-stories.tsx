		...
		<Card className="w-[375px] h-[667px] overflow-hidden relative rounded-md">
			<CardContent className="p-0 h-full">
				<div className="absolute top-0 left-0 right-0 z-10 flex p-2 gap-1">
					{stories.map((_, index) => (
						<div key={index} className="h-1 bg-muted-foreground flex-1 rounded-full overflow-hidden">
							<div
				...
				<div className="h-full flex items-center justify-center bg-background">
					<p className="text-2xl font-bold text-center px-8">{stories[currentStory].content}</p>
				</div>
			</CardContent>
		</Card>
	);
